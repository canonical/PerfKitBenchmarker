...
class OpenStackVirtualMachine(virtual_machine.BaseVirtualMachine):
    ...
    def __init__(self, vm_spec):
        ...
        self.network_name = FLAGS.openstack_network or FLAGS.openstack_private_network
    ...
    def _CheckFlavor(self):
        ...
        cmd = os_utils.OpenStackCLICommand(self, "flavor", "show", self.machine_type)
    ...
    def _CheckFloatingIPNetworkExists(self, floating_network_name_or_id):
        ...
        is_external = network.get("is_router_external", network.get("router:external"))
        if is_external not in ("External", True):
            raise errors.Config.InvalidValue(
    ...
    def _CheckNetworkExists(self, network_name_or_id):
        cmd = os_utils.OpenStackCLICommand(self, "network", "show", network_name_or_id)
    ...
    def _UploadSSHPublicKey(self):
        ...
        with self._lock:
            if self.zone in self.uploaded_keypair_set:
                return
            cmd = os_utils.OpenStackCLICommand(self, "keypair", "create", self.key_name)
    ...
    def _DeleteSSHPublicKey(self):
        ...
        with self._lock:
            if self.zone in self.deleted_keypair_set:
                return
            cmd = os_utils.OpenStackCLICommand(self, "keypair", "delete", self.key_name)
    ...
    def _CreateServerGroup(self, group_name):
        cmd = os_utils.OpenStackCLICommand(self, "server group", "create", group_name)
    ...
    def CreateScratchDisk(self, _, disk_spec):
        ...
        disks = [
            os_disk.OpenStackDisk(disk_spec, name, self.zone) for name in disks_names
        ]
