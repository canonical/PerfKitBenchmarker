...
def CreateBootVolume(resource, name, image):
    ...
    vol_cmd.flags["size"] = resource.disk_size or GetImageMinDiskSize(resource, image)
    stdout, _, _ = vol_cmd.Issue()
    vol_resp = json.loads(stdout)
    return vol_resp
...
def DeleteVolume(resource, volume_id):
    ...
    vol_cmd = os_utils.OpenStackCLICommand(
        resource, "volume", "set", "--state", "available", volume_id
    )
    del vol_cmd.flags["format"]  # volume set does not support json output
    vol_cmd.Issue()
    vol_cmd = os_utils.OpenStackCLICommand(resource, "volume", "delete", volume_id)
    del vol_cmd.flags["format"]  # volume delete does not support json output
    vol_cmd.flags["force"] = True
    vol_cmd.Issue()
...
class OpenStackDiskSpec(disk.BaseDiskSpec):
    ...
    def _ApplyFlags(cls, config_values, flag_values):
        ...
        elif flag_values["data_disk_size"].present:
