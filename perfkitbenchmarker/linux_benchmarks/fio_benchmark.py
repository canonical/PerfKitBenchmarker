...
flags.DEFINE_boolean("fio_hist_log", False, "Whether to collect clat histogram.")
flags.DEFINE_integer(
    "fio_log_hist_msec",
    1000,
...
ALL_ACCESS_PATTERNS = frozenset([ACCESS_PATTERN_SEQUENTIAL, ACCESS_PATTERN_RANDOM])
...
def GetScenarioFromScenarioString(scenario_string):
    ...
    if access_pattern not in ALL_ACCESS_PATTERNS:
        raise errors.Setup.InvalidFlagConfigurationError(
            f"Unexpected access pattern {access_pattern} in scenario {scenario_string}"
        )
...
def GenerateJobFileString(
    ...
    default_size_string = str(working_set_size) + "G" if working_set_size else "100%"
    blocksize_override = (
...
def CleanupVM(vm):
    ...
    if not AgainstDevice() and not FLAGS.fio_jobfile:
        ...
        vm.RemoveFile(
            posixpath.join(vm.GetScratchDir(), DEFAULT_TEMP_FILE_NAME)
        )  # up after themselves, because we don't know their temp file name.
