...
def _MergeNginxConfigs(global_conf_path, server_conf_path, force_http=False):
  ...
  Args:
    ...
    server_conf_path: Path to the server-specific config (nginx/rp_apigw.conf or
...
def _ProbeNginx(vm, url):
  ...
  if status not in ("200", "301", "302"):
    raise errors.Resource.RetryableGetError(f"Got HTTP {status} from {url}")
...
def _PrepareCluster(benchmark_spec):
  ...
  benchmark_spec.nginx_endpoint_ip = kubernetes_commands.GetLoadBalancerIP(
      "nginx-cluster"
  )
