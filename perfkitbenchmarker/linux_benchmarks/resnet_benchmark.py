...
flags.DEFINE_bool(
    'resnet_skip_host_call',
    False,
    'Skip the host_call which is '
...
flags.DEFINE_integer(
    ...
    'model only after finishing the entire training regime).',
    lower_bound=2,
)
...
def Prepare(benchmark_spec):
  ...
  if not benchmark_spec.tpus:
    ...
    vm.RemoteCommand(
        ...
        'gcloud storage -m cp --recursive {data_dir}/* {data_path}'.format(
...
def MakeSamplesFromEvalOutput(metadata, output, elapsed_seconds, use_tpu=True):
  ...
  return [
      ...
      sample.Sample(
          'Top 1 Accuracy', float(top_1_accuracy) * 100, '%', metadata_copy
      ),
      sample.Sample(
          'Top 5 Accuracy', float(top_5_accuracy) * 100, '%', metadata_copy
      ),
  ]
