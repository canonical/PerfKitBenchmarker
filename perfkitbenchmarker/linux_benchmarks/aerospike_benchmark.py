...
from perfkitbenchmarker import background_tasks, configs, disk, errors, sample, vm_util
from perfkitbenchmarker import benchmark_spec as bm_spec
from perfkitbenchmarker.linux_packages import aerospike_client, aerospike_server
...
def Prepare(benchmark_spec: bm_spec.BenchmarkSpec) -> None:
    ...
    if FLAGS.aerospike_enable_strong_consistency:
        for server in servers:
            aerospike_server.EnableStrongConsistency(server, AEROSPIKE_NAMESPACES.value)
    client_install_fns = [
...
def Run(benchmark_spec: bm_spec.BenchmarkSpec) -> List[sample.Sample]:
    ...
    for threads in range(
        ...
        if num_client_vms * FLAGS.aerospike_instances == 1:
            ...
            detailed_samples = aerospike_client.AggregateAsbenchSamples(stdout_samples)
